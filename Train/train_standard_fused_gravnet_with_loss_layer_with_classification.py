...
def gravnet_model(Inputs, feature_dropout=-1.):
    ...
    x_data, x_row_splits = RaggedConstructTensor(name="construct_ragged")([I_feat, I_splits])
    ...
    x = create_output_layers(x, x_row_splits, n_ccoords=n_ccoords, scale_exp_e=False, n_classes=6)

    truth_dict = create_ragged_cal_truth_dict(I_truth)
    pred_dict = create_ragged_cal_pred_dict(x, n_ccoords=n_ccoords, n_classes=6)
    feat_dict = create_ragged_cal_feature_dict(I_feat)

    x = LLObjectCondensation(name="ll_obc_cond", dynamic=True, n_classes=6, n_ccoords=n_ccoords)([x, truth_dict, pred_dict, feat_dict, x_row_splits])
