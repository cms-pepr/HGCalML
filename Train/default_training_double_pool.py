...
from LayersRagged import RaggedConstructTensor, RaggedGlobalExchange,FusedRaggedGravNet, FusedRaggedGravNet_simple, CondensateAndSum, RaggedSelectThreshold, VertexScatterer
...
def gravnet_model(Inputs, feature_dropout=-1.,momentum=0.6):
    ...
    x_p = Dense(128, activation='elu',name="dense_last_a2")(x)
    x = x_p
    
    n_neighbours = 256
    
    x,c_a = FusedRaggedGravNet(n_neighbours=n_neighbours,
                                 n_dimensions=2,
                                 n_filters=64,
                                 n_propagate=64,
                                 name='gravnet_oc_a')([x, x_row_splits])
                                 
    x = Dense(64, activation='elu',name="dense_last_b1")(x)
    x = Dense(64, activation='elu',name="dense_last_b2")(x)
    
    x,c_b = FusedRaggedGravNet(n_neighbours=n_neighbours,
                                 n_dimensions=2,
                                 n_filters=64,
                                 n_propagate=64,
                                 name='gravnet_oc_b')([x, x_row_splits])
    ...
    ccoords = Add()([ScalarMultiply(0.5)(ccoords), ScalarMultiply(0.25)(c_a), ScalarMultiply(0.25)(c_b)])
