...
def config_model(Inputs, td, debug_outdir=None, plot_debug_every=2000):
    ...
    for i in range(GRAVNET_ITERATIONS):
        ...
        if i == 0 or i == 4:
            ...
            x_hit = TranslationInvariantMP([64, 32, 16], activation='elu')([xgn_hit, gnnidx_hit, gndist_hit])
            x_track = TranslationInvariantMP([64, 32, 16], activation='elu')([xgn_track, gnnidx_track, gndist_track])

            [xgn, x, gncoords], rs  = ConcatRaggedTensors()([
                [xgn_track, x_track, gncoords_track],
                [xgn_hit, x_hit, gncoords_hit],
                rs_track, rs_hit])
            ...
            x = TranslationInvariantMP([64, 32, 16], activation='elu')([xgn, gnnidx, gndist])


        x = Concatenate()([xgn, x, gncoords])
        x = Dense(d_shape,
