...
from GravNetLayersRagged import RaggedGravNet, DynamicDistanceMessagePassing
...
def gravnet_model(Inputs, feature_dropout=-1.):
    ...
    for i in range(n_gravnet_layers):
        n_filters = 196
        n_propagate = 128
        n_propagate_dyn_dist = [64,32,16,8,4,2]
        n_neighbours = 32
        n_dim = 8
        #if n_dim < 2:
        #    n_dim = 2
        x,coords,neighbor_indices, neighbor_dists_sq = RaggedGravNet(n_neighbours=n_neighbours,
                                                                     n_dimensions=n_dim,
                                                                     n_filters=n_filters,
                                                                     n_propagate=n_propagate,
                                                                     name='gravnet_' + str(i))([x, x_row_splits])

        x = DynamicDistanceMessagePassing(n_propagate_dyn_dist)([x, neighbor_indices, neighbor_dists_sq])
