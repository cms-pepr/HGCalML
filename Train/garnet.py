...
def gravnet_model(Inputs,nclasses,nregressions,otheroption):
    ...
    for i in range(n_gravnet_layers):
        ...
        x, coord = GravNet(n_neighbours=40, n_dimensions=4, n_filters=64, n_propagate=16, also_coordinates=True)(x)
...
def garnet_model(Inputs,nclasses,nregressions,otheroption):
    ...
    for i in range(n_garnet_layers):
        ...
        x = BatchNormalization(momentum=0.6)(x)
        x = Multiply()([x,mask])
        x = GarNet(n_aggregators=8, n_filters=32, n_propagate=12, name = 'garnet_'+str(i),)(x)
        x = BatchNormalization(momentum=0.6)(x)
        x = Multiply()([x,mask])
        feats.append(x)
